...
object SoapMessageFactory {
    ...
    fun <T : Any> createMessage(
        ...
        if (mtomEnabled) {
            ...
            val soapPartXopContentType =
                HttpHeaders.ValueWithParameters(MediaTypes.APPLICATION_XOP_XML, mapOf("charset" to "UTF-8", "type" to (soapPartContentType?.value ?: MediaTypes.TEXT_XML)))
            message.soapPart.setMimeHeader(HttpHeaders.CONTENT_TYPE, soapPartXopContentType.toString())

            val mimeXopContentType = HttpHeaders.ValueWithParameters(
                MediaTypes.MULTIPART_RELATED,
                mapOf("type" to soapPartXopContentType.value, "start-info" to soapPartXopContentType.parameters["type"])
            )
