...
fun SOAPMessage.mimeContentType() = mimeHeaders.getHeader("Content-Type")?.firstOrNull()?.let { HttpHeaders.ValueWithParameters.parse(it) }

fun SOAPMessage.soapPartContentType() = soapPart.getMimeHeader("Content-Type")?.firstOrNull()?.let { HttpHeaders.ValueWithParameters.parse(it) }
